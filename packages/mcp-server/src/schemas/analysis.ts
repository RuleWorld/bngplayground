...
export const identifiabilityArgsSchema = z.object({
    ...
    n_grid: positiveInt.min(2).max(200).optional().describe('Grid points per parameter (default: 20; minimum 2)'),
    range_factor: finiteNumber.gt(1).optional().describe('Grid range factor (default: 10; must be greater than 1)'),
    reoptimize: z.boolean().optional().describe('Re-optimize nuisance params (default: true)'),
    alpha: finiteNumber.gt(0).lt(1).optional().describe('Confidence level (default: 0.95)'),
    max_reopt_eval: positiveInt.max(200).optional().describe('Maximum optimizer evaluations per profile grid point (default: 50; max 200)'),
    method: z.enum(simulationMethods).optional(),
    t_end: finiteNumber.nonnegative().optional(),
    n_steps: positiveInt.optional(),
    solver: z.enum(solverValues).optional(),
