...
main(int argc, char *argv[])
{
	...
	int i,j,k,n,v,w,argnum,ne,loops;
	int codetype,outcode;
	t_ver_sparse_rep *VR;
	t_adjl_sparse_rep *AR;
	t_embed_sparse_rep *ER;
	int nbr_c,nbr_e_obs,size_A,pos_in_A;
	unsigned long nin,nout,nplan;
        char *arg,sw;
	...
	while (TRUE)
	{
	    ...
	    if (k != 2*ne)
	    {
		fprintf(stderr,">E planarg: decoding error; nin=%lu\n",nin);
		fprintf(stderr,"n=%d nde=%d ne=%d loops=%d\n",
			n,sg.nde,ne,loops);
		exit(1);
	    }
		...
		if (verbose)
		    fprintf(stderr,"graph %lu: n=%d ne=%d planar\n",
			    nin,n,ne);
	    }
	    else
	    {
		...
		if (verbose)
		    fprintf(stderr,"graph %lu: n=%d ne=%d non-planar\n",
			    nin,n,ne);
	    }
	    FREES(VR);
	    FREES(AR);
	...
	if (!nowrite)
	{
            if (!quiet)
                fprintf(stderr,
                ">Z  %lu graphs read from %s, %lu written to %s; %3.2f sec.\n",
