...
describe('Functional Rate Bytecode Performance', () => {
    maybeIt('benchmarks Hill functions (Native vs JS)', async () => {
        ...
        // Allow margin for CI runner/sandbox variance — native should generally be
        // faster but noisy environments can flip the result.
        expect(nativeTime).toBeLessThan(jsTime * 1.80);
    });
});
