...
cd ./build
rm -rf *
$EMCMAKE cmake ../sundials -DCMAKE_INSTALL_PREFIX=install -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC_LIBS=ON -DEXAMPLES_ENABLE_C=OFF -DEXAMPLES_INSTALL=OFF -DBUILD_CVODES=ON -DENABLE_KLU=ON -DKLU_ROOT="$ORIG_DIR/_deps/install-emscripten-klu" -DCMAKE_PREFIX_PATH="$ORIG_DIR/_deps/install-emscripten-klu" -DKLU_WORKS=TRUE
$EMMAKE make -j4
cd ..
