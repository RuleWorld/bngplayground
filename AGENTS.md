...
- `npm run test:fast` - Hang-safe, narrow scope (excludes parity/CVODE/simulation). Use for structural changes.
- `npm run test:full:safe` - Hang-safe, full scientific scope (parity vs BNG2, CVODE). Use for number-affecting changes.
...
`tests/profile-everything.spec.ts` is the performance yardstick: it times parse /
network generation / SSA, captures the engine's deterministic PROFILE_DATA
op-count breakdown (canonicalize, findAllMaps, matchComponents,
applyTransformation, isDuplicateReaction, degeneracy, speciesDedup), builds a
2^N combinatorial stressor, reports species/reaction/heap stats, and writes
`profile-report.md` plus a matching `.json`.

- It is **excluded** from the default / `test:fast` / `test:full:safe` suites (it
  is slow and, defaulting to ODE, would pull CVODE into the narrow suite). It runs
  only via its own config:
  `node scripts/run_full_tests.mjs --config vitest.profile.config.ts`
- Config via env: `PROFILE_MULTISITE` (site counts → 2^N species),
  `PROFILE_SIM` (default `ode,ssa`; set `ssa` with `PROFILE_ODE_COMPARE=0` to stay
  pure-JS, no CVODE), `PROFILE_MODELS_DIR` (add real models), `PROFILE_REPEATS`,
  `PROFILE_OUT`.
- **LOCKED:** do not edit this file, its thresholds, or the perf model set in a PR
  that claims a performance win. It is the ruler, not a target.
...
**packages/mcp-server/ (MCP tool handlers):**
- Tool handlers must **call** engine functions from `@bngplayground/engine`, never
  reimplement engine logic inline. Engine/MCP parity is an enforced invariant.
...
├── .github/workflows/               # CI/CD and Jules agent workflows
...
- Jules AI agent workflows in `.github/workflows/` for autonomous code quality, performance, and correctness improvements

## Autonomous Agent Conventions

For the Jules agents in `.github/workflows/`:
- **Locked evaluator:** never edit a test, metric, benchmark, threshold, or the
  perf harness (`tests/profile-everything.spec.ts`) in the same PR that claims to
  satisfy it. Optimize the code, not the ruler.
- **Test scope by blast radius:** structural changes (types, docstrings) validate
  with `npm run test:fast`; anything that can change numbers validates with
  `npm run test:full:safe`. Never run raw `npm run test:full` (it hangs) and never
  disable the `scripts/run_full_tests.mjs` harness.
- **Engine/MCP parity:** `packages/mcp-server` handlers must call engine functions
  from `@bngplayground/engine`, never reimplement engine logic inline.
